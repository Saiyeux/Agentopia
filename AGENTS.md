...
- **上下文无关引擎**: 不靠把完整世界观/角色卡/历史塞进 prompt,而是每拍读取 SQLite 当前状态切片
...
- **`01_数据库设计.md`**: 定义所有数据表结构、字段白名单、时间模型
- **`02_引擎程序设计.md`**: 定义 Python 引擎模块划分、主循环、调度逻辑
- **`03_提示词与模型交互设计.md`**: 定义三种 LLM 角色的 Prompt 契约和 JSON schema
- **`04_上下文无关游戏引擎设计.md`**: 定义"固定系统提示 + 当前数据库切片 + 本拍任务"的核心架构原则
...
- 每次 LLM 调用只注入当前任务需要的 DB 状态切片,禁止全量加载世界、角色、记忆和历史
...
Agentopia 首先是一个可换题材的 AI 游戏引擎,不是酒馆聊天 Demo。任何题材都应该走同一条循环:

```
当前状态(SQLite) → AI 生成语义/意图/delta → 引擎校验 → 写回 SQLite → 下一拍重新读取状态
```

实现时必须把 prompt 分成:
- **固定系统提示**: 输出契约、引擎规则、LLM 身份职责
- **当前数据切片**: 当前 actor、当前场景、相关关系/记忆、最近少量事件
- **本拍任务**: 行动、裁定或事件演绎

不要把完整世界观、完整角色卡、完整历史、完整关系网塞给 LLM。角色和世界一直在变,所以每拍上下文必须来自 SQLite 的最新状态,而不是靠模型记忆延续。

### 1. 字段白名单机制

LLM 产出的 delta 只允许修改以下字段(见 `01_数据库设计.md` §6):
...
- **记忆分级检索**: 只取与当前任务相关的高 salience summary + 最近少量 episodic
...
- **所有 prompt 来自固定契约 + DB 状态切片**,不要全量塞上下文
